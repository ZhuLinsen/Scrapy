...
class ZhihuSpider(scrapy.Spider):
    ...
    def parse_user(self, response):
        ...
        yield Request(url=self.follow_url.format(user=result.get('url_token'), include=self.follow_query, limit=20, offset=0), callback=self.parse_follow)
        yield Request(url=self.followers_url.format(user=result.get('url_token'), include=self.followers_query, limit=20, offset=0), callback=self.parse_followers)
    ...
    def parse_follow(self, response):
        ...
        if 'paging' in results.keys() and results.get('paging').get('is_end') == False:
            ...
            yield Request(url=next_page, callback=self.parse_follow)
